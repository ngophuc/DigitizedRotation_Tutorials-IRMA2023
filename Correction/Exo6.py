    ...
    for i in range(height-1):#Row
        for j in range(width-1):#Col
            ...
            if intensity == 255:#white pixel
                ...
                setPixelValue(image_upsampling, 2*i, 2*j, 255)
                setPixelValue(image_upsampling, 2*i+1, 2*j, 255)
                setPixelValue(image_upsampling, 2*i, 2*j+1, 255)
                setPixelValue(image_upsampling, 2*i+1, 2*j+1, 255)
...
if(is_well_composed):
    ...
    img_regular = createRegularImage(img_binary)
